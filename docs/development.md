...
本文档面向维护者，记录本仓库的生成、验证、发布和打包流程。普通读者请从根目录 `README.md` 和 `site/index.html` 开始。
...
当前不提交 GitHub Actions workflow。发布静态站点时，把 `site/` 推到 `gh-pages` 分支，然后在 GitHub Pages 设置中选择 `gh-pages` / root：
...
如果仓库是 private，GitHub Pages 可用性取决于账号计划。
